    ...
    for item in values:
        if isinstance(item, str):
            ...
            out.append(Evidence(
                ...
                locator_value=(str(item.get("locator_value") or locator.get("value")) if (item.get("locator_value") or locator.get("value")) is not None else None),
...
def _parse_relations(value: Any) -> tuple[Relation, ...]:
    ...
    for raw in value:
        ...
        out.append(Relation(
            predicate=str(raw["predicate"]), target=str(raw["target"]),
...
def parse_note(path: Path, vault_root: Path | None = None) -> ParsedNote:
    ...
    title = str(fm.get("title") or next((m.group(2) for m in HEADING_RE.finditer(body) if len(m.group(1)) == 1), path.stem))
    ...
    return ParsedNote(
        ...
        content_hash=sha256_text(text), frontmatter=fm, body=body, semantic=semantic,
        object_id=object_id, object_type=object_type, title=title, aliases=aliases, tags=tags,
        ...
        relations=_parse_relations(fm.get("relations")), timeline=timeline,
    )
...
def validate_note(note: ParsedNote, relation_registry: dict[str, Any] | None = None) -> list[ValidationIssue]:
    ...
    if note.object_type not in SEMANTIC_TYPES:
        # Extension types are allowed when namespaced or PascalCase; warn, do not reject.
    ...
    for i, item in enumerate(note.timeline):
        if not isinstance(item, dict):
            issues.append(ValidationIssue(p, "invalid-timeline", f"timeline[{i}] must be a mapping")); continue
...
class KnowledgeIndex:
    ...
    def __enter__(self) -> "KnowledgeIndex": return self
    ...
    def upsert(self, note: ParsedNote) -> None:
        ...
        self.conn.executemany("INSERT OR IGNORE INTO navigation(source_path,target_ref) VALUES(?,?)", [(path,x) for x in note.wikilinks])
        if note.semantic:
            status = str(note.frontmatter.get("status") or "accepted")
            self.conn.execute("INSERT INTO objects(id,type,title,path,status,body,frontmatter_json) VALUES(?,?,?,?,?,?,?)",
                              (note.object_id,note.object_type,note.title,path,status,note.body,json.dumps(note.frontmatter,default=str,sort_keys=True)))
            for alias in (note.title, *note.aliases):
                self.conn.execute("INSERT OR IGNORE INTO aliases(alias_norm,object_id,alias) VALUES(?,?,?)", (_norm(alias),note.object_id,alias))
            self.conn.execute("INSERT INTO objects_fts(id,title,body,aliases,tags) VALUES(?,?,?,?,?)",
                              (note.object_id,note.title,note.body," ".join(note.aliases)," ".join(note.tags)))
            for rel in note.relations:
                cur = self.conn.execute("INSERT INTO relations(source_id,predicate,target_id,derivation,status,event_time,transaction_time,confidence,path) VALUES(?,?,?,?,?,?,?,?,?)",
                    (note.object_id,rel.predicate,rel.target,rel.derivation,rel.status,rel.event_time,rel.transaction_time,rel.confidence,path))
                rid = int(cur.lastrowid)
                self.conn.executemany("INSERT INTO evidence(relation_id,source,locator_type,locator_value,authority) VALUES(?,?,?,?,?)",
            ...
            for i,item in enumerate(note.timeline):
                if isinstance(item,dict) and all(item.get(k) for k in ("event_time","transaction_time","claim","source")):
                    self.conn.execute("INSERT INTO timeline(object_id,ordinal,event_time,transaction_time,claim,source) VALUES(?,?,?,?,?,?)",
                        (note.object_id,i,str(item["event_time"]),str(item["transaction_time"]),str(item["claim"]),str(item["source"])))
        self.conn.commit()
    ...
    def build(self, vault_root: Path, schema_root: Path) -> list[ValidationIssue]:
        ...
        for path in iter_markdown(vault_root):
            ...
            p = str(note.path).replace("\\","/"); seen_paths.add(p)
        ...
        for p in stale: self._remove_path(p)
        self.conn.commit()
        return issues

    def rebuild(self, vault_root: Path, schema_root: Path) -> list[ValidationIssue]:
        self.close()
        with contextlib.suppress(FileNotFoundError): self.db_path.unlink()
        self.conn = sqlite3.connect(self.db_path); self.conn.row_factory=sqlite3.Row; self.conn.executescript(SCHEMA_SQL)
    ...
    def resolve(self, ref: str) -> list[dict[str, Any]]:
        direct = self.get(ref)
        if direct: return [{"id":ref,"title":direct["title"],"match":"id","score":1.0}]
        rows = self.conn.execute("SELECT o.id,o.title,a.alias FROM aliases a JOIN objects o ON o.id=a.object_id WHERE a.alias_norm=?",(_norm(ref),)).fetchall()
        if rows: return [{"id":r["id"],"title":r["title"],"match":"alias","score":1.0} for r in rows]
        needle = _norm(ref)
        candidates=[]
    ...
    def search(self, query: str, limit: int = 20) -> list[dict[str, Any]]:
        try:
            rows=self.conn.execute("SELECT id,title,snippet(objects_fts,2,'[',']',' … ',12) AS snippet FROM objects_fts WHERE objects_fts MATCH ? LIMIT ?",(query,limit)).fetchall()
        except sqlite3.OperationalError:
            q=f"%{query.lower()}%"
            rows=self.conn.execute("SELECT id,title,substr(body,1,240) AS snippet FROM objects WHERE lower(title) LIKE ? OR lower(body) LIKE ? LIMIT ?",(q,q,limit)).fetchall()
    ...
    def neighbors(self, object_id: str, predicate: str | None=None, include_derived: bool=False) -> list[dict[str,Any]]:
        ...
        if not include_derived: sql += " AND status='accepted' AND derivation!='inferred'"
    ...
    def trace(self, start: str, end: str, max_depth: int=6) -> list[str]:
        ...
        while q:
            ...
            rows=self.conn.execute("SELECT source_id,target_id FROM relations WHERE status='accepted' AND (source_id=? OR target_id=?)",(node,node))
            for r in rows:
    ...
    def timeline(self, object_id: str) -> list[dict[str,Any]]:
        return [dict(r) for r in self.conn.execute("SELECT * FROM timeline WHERE object_id=? ORDER BY event_time,transaction_time,ordinal",(object_id,))]

    def sources(self, object_id: str) -> list[dict[str,Any]]:
        return [dict(r) for r in self.conn.execute("SELECT r.source_id,r.predicate,r.target_id,e.source,e.locator_type,e.locator_value,e.authority FROM relations r JOIN evidence e ON e.relation_id=r.id WHERE r.source_id=? OR r.target_id=?",(object_id,object_id))]

    def contradiction_candidates(self) -> list[dict[str,Any]]:
        rows=self.conn.execute("SELECT a.source_id,a.predicate,a.target_id AS left_target,b.target_id AS right_target,a.event_time,a.id AS left_id,b.id AS right_id FROM relations a JOIN relations b ON a.source_id=b.source_id AND a.predicate=b.predicate AND a.id<b.id WHERE a.status='accepted' AND b.status='accepted' AND a.target_id<>b.target_id AND COALESCE(a.event_time,'')=COALESCE(b.event_time,'')")
        return [dict(r) for r in rows]
    ...
    def health(self) -> dict[str,Any]:
        ...
        for r in self.conn.execute("SELECT id,source_id,predicate,target_id FROM relations"):
            if r["target_id"] not in ids: unresolved.append(dict(r))
        return {"files":count("files"),"objects":count("objects"),"relations":count("relations"),"navigation_edges":count("navigation"),"timeline_entries":count("timeline"),"unresolved_relation_targets":unresolved,"contradiction_candidates":self.contradiction_candidates()}
...
def validate_vault(vault_root: Path) -> list[ValidationIssue]:
    _,schema,_=vault_roots(vault_root); registry=load_relation_registry(schema); issues=[]; ids={}
    for path in iter_markdown(vault_root):
        try: note=parse_note(path,vault_root)
        except Exception as exc: issues.append(ValidationIssue(str(path.relative_to(vault_root)),"parse-error",str(exc))); continue
        issues.extend(validate_note(note,registry))
        if note.semantic and note.object_id:
            if note.object_id in ids: issues.append(ValidationIssue(str(note.path),"duplicate-id",f"also used by {ids[note.object_id]}"))
            ids[note.object_id]=str(note.path)
    return issues


def propose_frontmatter_patch(path: Path, patch: dict[str,Any]) -> dict[str,Any]:
    text=path.read_text(encoding="utf-8"); before_hash=sha256_text(text); fm,body=split_frontmatter(text)
    candidate=dict(fm); candidate.update(patch)
...
def validate_patch(proposal: dict[str,Any], vault_root: Path) -> list[ValidationIssue]:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        test=Path(tmp)/path.name; test.write_text(str(proposal["content"]),encoding="utf-8")
        note=parse_note(test)
        _,schema,_=vault_roots(vault_root)
        return validate_note(note,load_relation_registry(schema))
...
def apply_patch(proposal: dict[str,Any], vault_root: Path) -> None:
    ...
    try:
        with os.fdopen(fd,"w",encoding="utf-8",newline="") as fh: fh.write(str(proposal["content"])); fh.flush(); os.fsync(fh.fileno())
        os.replace(tmp,path)
    finally:
...
def export_okf_like(index: KnowledgeIndex, output: Path) -> None:
    """Conservative OKF-oriented JSON export scaffold; not a claim of full OKF conformance."""
    objects=[]
    for row in index.conn.execute("SELECT id,type,title,path,status,frontmatter_json FROM objects ORDER BY id"):
        item=dict(row); item["frontmatter"]=json.loads(item.pop("frontmatter_json")); item["relations"]=index.get(item["id"])["relations"]; objects.append(item)
    output.parent.mkdir(parents=True,exist_ok=True)
    output.write_text(json.dumps({"format":"agentic-vault-okf-projection","version":"0.1.0","objects":objects},indent=2,ensure_ascii=False),encoding="utf-8")
